...
class E2ELauncher {
  ...
  async startBackendServer() {
    ...
    try {
      await this.healthCheck('http://localhost:5000/health');
    ...
    backendProcess.stdout.on('data', (data) => {
      ...
      if (output.includes('Server running on port') || output.includes('server running on port') || output.includes('Database connection verified')) {
