...
async function apiFetch(endpoint, options = {}) {
  // Build URL correctly:
  // - If using external API (Railway): strip /api prefix from endpoint
  // - If using local API (/api): keep /api prefix in endpoint
  let url;
  if (process.env.NEXT_PUBLIC_API_URL) {
    // External API: endpoint comes with /api prefix, need to strip it
