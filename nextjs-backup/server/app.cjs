...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    try {
      ...
      if (allowedOrigins.has(origin) || isVercel) {
...
app.get('/health', async (req, res) => {
  try {
    // Test database connection when enabled
    if (!db.disabled) {
    ...
    res.json({
      ...
      database: db.disabled ? 'disabled' : 'connected',
      version: '1.0.0',
    ...
    res.status(503).json({
      status: 'unhealthy',
      timestamp: new Date().toISOString(),
      database: db.disabled ? 'disabled' : 'disconnected',
...
const server = app.listen(PORT, () => {
  ...
  db.testConnection().then(() => {
    if (db.disabled) {
      console.log(`ℹ️ Database disabled; running in fallback mode`);
    } else {
