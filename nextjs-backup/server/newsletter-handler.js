...
class NewsletterHandler {
  ...
  async handleSubscription(req, res) {
    try {
      ...
      // Validate request
      const validation = this.validateSubscriptionRequest(email);
      if (!validation.valid) {
        return this.sendErrorResponse(res, 400, validation.error);
      }

      // Check rate limiting
      const rateLimitCheck = this.checkRateLimit(email, _clientIP);
      ...
      if (result.success) {
        ...
        this.updateRateLimit(email, _clientIP, true);
        ...
        this.updateRateLimit(email, _clientIP, false);
  ...
  validateSubscriptionRequest(email) {
    // Email validation
  ...
  updateRateLimit(email, clientIP, _success) {
    const now = Date.now();
