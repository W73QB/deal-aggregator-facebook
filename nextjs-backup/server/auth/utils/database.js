...
class DatabaseConnection {
  constructor() {
    ...
    if (!this.connectionString) {
      console.warn('DATABASE_URL not set; database operations will run in disabled mode with fallbacks.');
      this.disabled = true;
      this.pool = null;
      return;
    }
  ...
  async getClient() {
    try {
      if (this.disabled || !this.pool) {
        throw new Error('Database connection is disabled (no DATABASE_URL).');
      }
  ...
  async testConnection() {
    if (this.disabled) {
      console.warn('Database disabled; skipping connection test');
      this.connected = false;
