...
CREATE TABLE IF NOT EXISTS deals (
    ...
    description TEXT,
    image_url TEXT, -- API expects 'image' - will map in API code
    original_price DECIMAL(10,2),
    sale_price DECIMAL(10,2), -- Added: API expects this
    discount_percentage INTEGER, -- API expects 'discount' - will map in API code
    rating NUMERIC(2,1), -- Added: API expects this (0.0-5.0 rating)
    category VARCHAR(100),
    featured BOOLEAN DEFAULT FALSE, -- Added: API expects this
    store TEXT, -- Added: API expects this (different from source_platform)
    affiliate_url TEXT,
    tags TEXT, -- Added: API expects this
    stock_count INTEGER, -- Added: API expects this

    -- Product details (keeping existing)
    brand VARCHAR(100),
    model VARCHAR(200),
    condition VARCHAR(50) DEFAULT 'new', -- new, refurbished, open-box

    -- Links and sources (keeping existing)
    deal_url TEXT NOT NULL,
    source_platform VARCHAR(50), -- amazon, facebook, walmart, etc
    source_id VARCHAR(200), -- external platform ID

    -- Status and tracking
    active BOOLEAN DEFAULT TRUE, -- Added: API expects this for WHERE filtering
    status VARCHAR(20) DEFAULT 'active', -- Keeping existing: active, expired, sold_out, draft
    ...
    expires_at TIMESTAMP WITH TIME ZONE, -- Added: API expects this
    deal_end_date TIMESTAMP WITH TIME ZONE, -- Keeping existing
...
CREATE INDEX IF NOT EXISTS idx_deals_sale_price ON deals(sale_price); -- Added for API queries
CREATE INDEX IF NOT EXISTS idx_deals_discount ON deals(discount_percentage DESC);
CREATE INDEX IF NOT EXISTS idx_deals_featured ON deals(featured); -- Added for API queries
CREATE INDEX IF NOT EXISTS idx_deals_rating ON deals(rating DESC); -- Added for API queries
CREATE INDEX IF NOT EXISTS idx_deals_active ON deals(active); -- Added for API WHERE clauses
CREATE INDEX IF NOT EXISTS idx_deals_expires_at ON deals(expires_at); -- Added for API WHERE clauses
